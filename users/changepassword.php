...
if(isset($_POST['btnsubmit'])){
	...
	if ($newpass !== $confirm_newpass) {
        $msg = "New password and confirm new password do not match.";
			...
			// Get the logged-in user's ID (assuming you have a session)
			$user_id = $_SESSION['user_id'];

		$sql0 = "SELECT * FROM users WHERE id = '$user_id' AND password = '$oldpass'";
		$res0 = mysqli_query($conn, $sql0) or die(mysqli_error());
		$num0 = mysqli_num_rows($res0);
		if($num0 == 0){
			echo "Incorrect Password";
		}else{
			$sql1 = "UPDATE users SET password = '$newpass' WHERE id = '$user_id'";
